...
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Открыть"),"",tr("C++ файл (*.cpp *.h)"));
    if(fileName!="")
    {
        QFile file(fileName);
        if(!file.open(QIODevice::ReadOnly))
        {
            QMessageBox::critical(this, tr("Ошибка"), tr("Файл невозможно открыть!"));
            return;
        }
...
{
    ui->label_averNumMethLines->setText("Среднее количество строк кода на метод: " + QString::number(averNumMethLines(code)));
    ui->label_averNumModuleLines->setText("Среднее количество строк кода на модуль: " + QString::number(averNumModuleLines(code)));
    ui->label_averNumClassLines->setText("Среднее количество строк кода на класс: " + QString::number(averNumClassLines(code)));
    ui->label_NORM->setText("Количество вызовов удаленых методов: " + QString::number(NORM(code)));
    ui->label_RFC->setText("Отклик на класс: " + QString::number(RFC(code)));
    ui->label_WMC->setText("Суммарная сложность всех методов класса: " + QString::number(WMC(code)));
    MainWindow::NOC(code);
}
